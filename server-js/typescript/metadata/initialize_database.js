...
module.exports = function (next) {
    var metadata_table_sql = "CREATE TABLE IF NOT EXISTS Pages (" +
        ...
        //"parent TEXT" +
        ");";
    query(metadata_table_sql, [], function (err, res) {
        if (err)
            throw new Error(err);
        ...
        var revision_table_sql = "CREATE TABLE IF NOT EXISTS Revisions (" +
            ...
            ");";
        query(revision_table_sql, [], function (err, res) {
            if (err)
                throw new Error(err);
            ...
            var rating_table_sql = "CREATE TABLE IF NOT EXISTS Ratings (" +
                ...
                ");";
            query(rating_table_sql, [], function (err, res) {
                if (err)
                    throw new Error(err);
                var author_table_sql = "CREATE TABLE IF NOT EXISTS Authors (" +
                    ...
                    "created_at TIMESTAMP NOT NULL" +
                    ");";
                query(author_table_sql, [], function (err, res) {
                    if (err)
                        throw new Error(err);
                    var file_table_sql = "CREATE TABLE IF NOT EXISTS Files (" +
                        ...
                        ");";
                    query(file_table_sql, [], function (err, res) {
                        if (err)
                            throw new Error(err);
                        var parent_table_sql = "CREATE TABLE IF NOT EXISTS Parents (" +
                            ...
                            ");";
                        query(parent_table_sql, [], function (err, res) {
                            if (err)
                                throw new Error(err);
                            next(0);
                        });
